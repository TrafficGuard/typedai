import { TestBed, fakeAsync, tick, effect } from '@angular/core/testing';
...
describe('WorkflowsService', () => {
  ...
  describe('loadRepositories', () => {
    it('should fetch repositories and update repositoriesState on success', (done) => {
      ...
      effect(() => {
        ...
        callCount++;
        if (callCount === 1) { // Initial state by createApiListState
          expect(state.status).toBe('idle');
        } else if (callCount === 2) { // Loading state after service.loadRepositories()
          expect(state.status).toBe('loading');
        } else if (callCount === 3) { // Success state after flush
  ...
  describe('runCodeEditorImplementRequirements', () => {
    it('should POST to /api/workflows/edit and return data on success', (done) => {
      ...
      service.runCodeEditorImplementRequirements(mockPayload.workingDirectory, mockPayload.requirements)
        .subscribe(response => {
  ...
  describe('runCodebaseQuery', () => {
    it('should POST to /api/workflows/query and return data on success', (done) => {
      ...
      service.runCodebaseQuery(mockPayload.workingDirectory, mockPayload.query)
        .subscribe(response => {
          expect(response).toEqual(mockResponse);
          done();
        });
  ...
  describe('selectFilesToEdit', () => {
    it('should POST to /api/workflows/select-files and return data on success', (done) => {
      ...
      service.selectFilesToEdit(mockPayload.workingDirectory, mockPayload.requirements)
        .subscribe(response => {
          expect(response).toEqual(mockResponse);
          done();
        });
