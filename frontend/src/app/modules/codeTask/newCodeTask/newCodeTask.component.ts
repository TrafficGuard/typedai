...
export class NewCodeTaskComponent implements OnInit, OnDestroy {
	...
	// State for presets
	presets$: Observable<CodeTaskPreset[]> = of([]);
	...
	fetchRepositories(): void {
		...
		this.localRepos$ = this.codeTaskService.getRepositories().pipe(
			catchError((err) => {
	...
	loadPresets(): void {
		...
		this.presets$ = this.codeTaskService.listCodeTaskPresets().pipe(
			catchError((err) => {
	...
	applyPreset(preset: CodeTaskPreset): void {
		if (!preset) return;
	...
	savePreset(): void {
		...
		this.codeTaskService
			.saveCodeTaskPreset(presetName, config)
			.pipe(
				finalize(() => {
					this.isSubmitting = false;
				}),
	...
	onSubmit(): void {
		...
		const payload: CreateCodeTaskPayload = {
		...
		console.log('Submitting Code Task Payload:', payload);

		this.codeTaskService
			.createCodeTask(payload)
			.pipe(
				finalize(() => {
					this.isSubmitting = false;
				}),
			...
			.subscribe({
				next: (createdCodeTask: CodeTask) => {
					console.log('Code task created:', createdCodeTask);
					this.router.navigate(['/ui/codeTask', createdCodeTask.id]);
				},
				error: (err) => {
					this.snackBar.open(`Error creating Code task: ${err.message || 'Unknown error'}`, 'Close', {
						duration: 5000, // Show for 5 seconds
						verticalPosition: 'top', // Position at the top
					});
					console.error('Error creating Code task:', err); // Keep the console log for developer debugging purposes
				},
			});
	}
}
