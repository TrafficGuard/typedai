...
@Component({
	...
	standalone: true,
	imports: [CommonModule, DatePipe, JsonPipe, KeyValuePipe, MatExpansionModule, MatCardModule, MatTooltipModule],
...
export class AgentIterationsComponent {
	...
	readonly reversedIterations = computed(() => {
		...
		return details?.iterations ? details.iterations.slice().reverse() : [];
	});
