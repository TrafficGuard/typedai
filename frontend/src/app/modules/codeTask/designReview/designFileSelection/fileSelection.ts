...
import { distinctUntilChanged, finalize, of, switchMap, take } from 'rxjs';
...
import { DesignFileTreeDialogComponent } from '../designFileTreeDialog/designFileTreeDialog.component';
...
export class FileSelection {
					...
					this.codeTaskService
						...
						.subscribe({
							next: (fileSystemNode) => {
								...
								this.editableFileSelection.set(currentCodeTask.fileSelection ? JSON.parse(JSON.stringify(currentCodeTask.fileSelection)) : []);
							},
							error: (err) => {
	...
	deleteFile(file: SelectedFile): void {
		if (!file.readOnly) {
			this.editableFileSelection.update((currentSelection) => currentSelection.filter((f) => f.filePath !== file.filePath));
	...
	onCategoryChange(file: SelectedFile, newCategory: SelectedFile['category']): void {
		this.editableFileSelection.update((currentSelection) => currentSelection.map((f) => (f.filePath === file.filePath ? { ...f, category: newCategory } : f)));
	...
	onBrowseFiles(): void {
		// Renamed from handleBrowseFilesRequest to match template
		if (!this.rootNode()) {
	...
	onSaveFileSelectionChanges(): void {
		...
		this.codeTaskService
			.updateCodeTask(codeTaskId, { fileSelection: selectionPayload })
			.pipe(
				take(1),
				finalize(() => {
			...
			.subscribe({
				next: (updatedCodeTask) => {
					// Assuming backend returns the updated codeTask
	...
	approveSelection(): void {
		...
		const saveIfNeeded$ = this.hasUnsavedChanges() ? this.codeTaskService.updateCodeTask(codeTaskId, { fileSelection: selectionPayload }) : of(null); // Using of(null) to create an observable that completes
	...
	submitFileUpdateInstructions(): void {
		...
		this.codeTaskService
			.updateFileSelection(codeTaskId, prompt)
			.pipe(
				take(1),
				finalize(() => {
					this.isProcessingAction.set(false);
	...
	onResetSelection(): void {
		...
		if (
			confirm(
				'Are you sure you want to reset the file selection? Any manual changes to the file list (additions, deletions) since the last AI update will be lost, and the list will revert to what the AI last provided for review.',
			)
		) {
