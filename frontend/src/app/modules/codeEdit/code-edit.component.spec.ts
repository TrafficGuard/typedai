...
describe('CodeEditComponent', () => {
	...
	it('should call services and navigate on successful submission', async () => {
		// GIVEN
		...
		component.selectedFiles.set(selectedFiles);
		await po.setInstructions(instructions);

		// WHEN
		await po.clickSubmit();

		// THEN
		expect(await po.isSubmitting()).toBeTrue();
		expect(fakeCodeEditService.getFilesContent).toHaveBeenCalledWith(selectedFiles);
		expect(fakeChatService.createChat).toHaveBeenCalledWith(jasmine.any(String), 'deepseek-coder');
		expect(fakeChatService.createChat.calls.mostRecent().args[0]).toContain(instructions);
		expect(fakeChatService.createChat.calls.mostRecent().args[0]).toContain('File: `root/package.json`');

		await fixture.whenStable();
		expect(routerSpy.navigate).toHaveBeenCalledWith(['/ui/chat', newChat.id]);
		expect(await po.isSubmitting()).toBeFalse();
	});

	it('should display an error message if createChat call fails', async () => {
		// GIVEN
		const selectedFiles = ['root/package.json'];
		const fileContents: FilesContentResponse = { 'root/package.json': '{}' };
		fakeCodeEditService.getFilesContent.and.returnValue(of(fileContents));
		fakeChatService.createChat.and.returnValue(throwError(() => new Error('API Error')));
		component.selectedFiles.set(selectedFiles);
		await po.setInstructions('Do something');
		...
		// THEN
		expect(await po.getSubmissionError()).toBe('Failed to create chat. Please try again.');
		expect(await po.isSubmitting()).toBeFalse();
		expect(await po.isSubmitButtonDisabled()).toBeFalse();
