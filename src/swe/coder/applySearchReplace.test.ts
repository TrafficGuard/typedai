...
describe('ApplySearchReplace', () => {
	...
	describe('EditBlockParser._findFilename (static private method)', () => {
		...
		it('should find filename on immediate preceding line', () => {
			...
			expect((EditBlockParser as any)._findFilename(content, '```')).to.equal('path/to/file.ts');
		});
		...
		it('should return undefined if no filename found in relevant lines', () => {
			...
			expect((EditBlockParser as any)._findFilename(content, '```')).to.equal(undefined);
		});
	...
	describe('EditBlockParser.findOriginalUpdateBlocks (static public method)', () => {
		const SEARCH_MARKER = '<<<<<<< SEARCH';
		const DIVIDER_MARKER = '=======';
		const REPLACE_MARKER = '>>>>>>> REPLACE';
		...
		it('should parse a single valid block', () => {
			...
			const edits = EditBlockParser.findOriginalUpdateBlocks(response, ['```', '```']);
