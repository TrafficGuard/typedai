...
export class DockerController {
    ...
    private generateProjectName(composeFilePath: string): string {
        return path.basename(path.dirname(composeFilePath)).replace(/[^a-zA-Z0-9_.-]/g, '').toLowerCase() || 'defaultproject';
    }
    ...
    private async runDockerComposeCommand(
        ...
        try {
            ...
            if (stderr) {
                ...
                const warningLines = stderrLines.filter(line => 
                    line.toLowerCase().includes('warning') || 
                    line.toLowerCase().includes('deprecated') ||
                    !stdout.includes(line) // If stderr content is not in stdout, it's more likely a real message
                );
                if (warningLines.length > 0 && warningLines.length === stderrLines.length) { // All stderr lines are warnings/info
                     if (!options?.logOutput && warningLines.join('\n').length > 0) logger.warn(`Docker compose command stderr (warnings/info):\n${warningLines.join('\n')}`);
    ...
    async waitForLogMessage(
        ...
        options?: { tail?: string | number }
    ): Promise<void> {
        return new Promise((resolve, reject) => {
            logger.info(`Waiting for log message matching ${messagePattern.source} from service ${serviceName} (project ${this.projectName}, timeout: ${timeoutMs}ms)`);
            let logProcess: ChildProcess | null = null;
            
            const timer = setTimeout(() => {
                logProcess?.kill();
                reject(new Error(`Timeout (${timeoutMs}ms) waiting for log message "${messagePattern.source}" from service ${serviceName} in project ${this.projectName}`));
            }, timeoutMs);
            ...
            logProcess.on('error', (err) => { // Handle error from spawn itself
                clearTimeout(timer);
            ...
            logProcess.on('close', (code) => {
                ...
                if (code !== 0 && code !== null) { // Check if it's still pending
                    const state = (logProcess as any)._pending; // Not standard, but a way to check
                    if (state !== 'killed') { // if not killed by us
                        clearTimeout(timer);
