...
// import { type GenerateTextOptions, type LLM, type LlmMessage, type UserContentExt, lastText, messageText, system, user } from '#shared/llm/llm.model';
//
...
//
//     constructor(
//         name: string = 'ConfidentDiverseDebateLLM',
...
//         return (
//             this.primaryLLM.isConfigured() &&
//             this.mediatorLLM.isConfigured() &&
//             this.debateLLMs.every((llm) => llm.isConfigured())
//         );
//     }
//
//     getModel(): string {
...
//     }
//
//     protected async generateTextFromMessages(
//         llmMessages: ReadonlyArray<LlmMessage>,
//         opts?: GenerateTextOptions,
//     ): Promise<string> {
//         logger.info(`[${this.name}] Phase 1: Initial Confidence Check with ${this.primaryLLM.getDisplayName()}`);
//
...
//             logger.info(`[${this.name}] High confidence in initial response. Skipping debate.`);
...
//         }
//
//         logger.info(`[${this.name}] Low confidence in initial response. Proceeding to debate.`);
//         logger.info(`[${this.name}] Phase 2: Generating initial diverse responses.`);
//         const initialDiverseResponses = await this.generateInitialDiverseResponses(llmMessages, opts);
//
//         logger.info(`[${this.name}] Phase 3: Debating responses for ${DEBATE_ROUNDS -1} rounds.`);
//         const debatedResponses = await this.multiAgentSparseDebate(
//             llmMessages,
//             initialDiverseResponses,
//             opts,
//             DEBATE_ROUNDS,
//         );
//
//         logger.info(`[${this.name}] Phase 4: Mediating final response with ${this.mediatorLLM.getDisplayName()}.`);
...
//
//                     let debatePromptContent: LlmMessage[] = [
//                         system(
...
//                     ];
//
//                     // If the originalMessages contained a system prompt, we should try to preserve it or integrate it.
//                     // For simplicity here, we are creating a new context for debate.
//                     // A more advanced version might try to merge system prompts.
//                     const debateContextMessages: LlmMessage[] = [{role: 'user', content: debatePromptContent}];
//
//
...
//         const originalUserQuery = messageText(originalMessages[originalMessages.length - 1]);
//
//         const mediationPromptContent: UserContentExt = [
//             system(
...
//         ];
//
//         const mediationContextMessages: LlmMessage[] = [{role: 'user', content: mediationPromptContent}];
//
//
