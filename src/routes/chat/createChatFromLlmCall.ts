...
export function createChatFromLlmCallRoute(fastify: AppFastifyInstance): void {
	registerApiRoute(fastify, CHAT_API.createChatFromLlmCall, async (req, reply) => {
		...
		if (!llmCall) {
			return sendNotFound(reply, 'LLM call not found.');
		}

		const user = currentUser();
		if (llmCall.userId !== user.id) {
			return sendErrorResponse(reply, 403, 'You do not have permission to access this LLM call.');
		}
