...
export class PatchUtils {
	static _stripQuotedWrapping(text: string, filename?: string, fencePair?: [string, string]): string {
	...
	static _prep(content: string): { text: string; lines: string[] } {
	...
	static _perfectReplace(wholeLines: string[], partLines: string[], replaceLines: string[]): string | undefined {
	...
	static _matchButForLeadingWhitespace(wholeChunkLines: string[], partLines: string[], lenientLeadingWhitespace: boolean): string | undefined {
	...
	static _replacePartWithMissingLeadingWhitespace(
		wholeLines: string[],
		partLines: string[],
		replaceLines: string[],
		...
		for (let i = 0; i <= wholeLines.length - normPartLines.length; i++) {
			...
			const leadingWsToAdd = this._matchButForLeadingWhitespace(wholeChunk, normPartLines, lenientLeadingWhitespace);
	...
	static _escapeRegExp(str: string): string {
		return str.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
	}

	static _tryDotDotDots(wholeContentStr: string, partContentStr: string, replaceContentStr: string): string | undefined {
		const dotsRegex = /(^\s*\.\.\.\s*\n)/m;
	...
	static _replaceMostSimilarChunk(whole: string, part: string, replace: string, lenientLeadingWhitespace: boolean): string | undefined {
		...
		result = this._replacePartWithMissingLeadingWhitespace(wholeLines, partLines, replaceLines, lenientLeadingWhitespace);
		if (result !== undefined) return result;
		...
		if (partLines.length > 0 && partLines[0].trim() === '') {
			...
			if (skippedBlankPartLines.length > 0) {
				...
				if (result !== undefined) return result;
				result = this._replacePartWithMissingLeadingWhitespace(wholeLines, skippedBlankPartLines, replaceLines, lenientLeadingWhitespace);
				if (result !== undefined) return result;
			}
		}
	...
	static _doReplace(
		...
		if (!beforeText.trim()) {
			const base = currentContent as string;
		...
		return this._replaceMostSimilarChunk(currentContent as string, beforeText, afterText, lenientLeadingWhitespace);
	}
}
