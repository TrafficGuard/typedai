...
import { sendBadRequest, sendNotFound, sendJSON } from '#fastify/responses'; // sendJSON is on reply in original spec, but is a standalone function
...
export async function promptRoutes(fastify: AppFastifyInstance) {
     ...
     */
    fastify.get(
        PROMPT_API.listPrompts.pathTemplate,
        { schema: PROMPT_API.listPrompts.schema },
        async (req, reply) => {
            const userId = currentUser().id;
            try {
                // As per requirement: service returns PromptList { prompts: PromptPreview[], hasMore: boolean }
                const promptList = await fastify.promptsService.listPrompts(userId);
     ...
     */
    fastify.post(
        PROMPT_API.createPrompt.pathTemplate,
        { schema: PROMPT_API.createPrompt.schema },
        async (req, reply) => {
            const payload = req.body as Static<typeof PromptCreateSchema>;
            const userId = currentUser().id;

            // As per requirement: construct Prompt object and pass to service
            // This implies the service method `createPrompt` takes this fully-formed object.
            const promptToCreate: Prompt = {
                id: randomUUID(), // Handler generates ID as per requirement's construction
                userId,
                parentId: payload.parentId,
                revisionId: 1, // Initial revision as per requirement
                name: payload.name,
            ...
            try {
                const createdPrompt = await fastify.promptsService.createPrompt(promptToCreate);
                // The schema for response is PromptSchema, so cast to PromptSchemaModel
                reply.code(201);
                sendJSON(reply, createdPrompt as PromptSchemaModel);
            } catch (error: any) {
                logger.error({ err: error, userId, payload: promptToCreate }, 'Error creating prompt');
     ...
     */
    fastify.get(
        PROMPT_API.getPromptById.pathTemplate,
        { schema: PROMPT_API.getPromptById.schema },
        async (req, reply) => {
            ...
            try {
                ...
                const prompt = await fastify.promptsService.getPromptById(promptId, userId);
                if (!prompt) {
     ...
     */
    fastify.get(
        PROMPT_API.getPromptRevision.pathTemplate,
        { schema: PROMPT_API.getPromptRevision.schema },
        async (req, reply) => {
            ...
            try {
                ...
                const prompt = await fastify.promptsService.getPromptRevision(promptId, revisionId, userId);
                if (!prompt) {
     ...
     */
    fastify.patch(
        PROMPT_API.updatePrompt.pathTemplate,
        { schema: PROMPT_API.updatePrompt.schema },
        async (req, reply) => {
            const { promptId } = req.params as Static<typeof PromptParamsSchema>;
            ...
            try {
                ...
                const updatedPrompt = await fastify.promptsService.updatePrompt(promptId, serviceUpdates, userId);
                // The schema for response is PromptSchema, so cast to PromptSchemaModel
     ...
     */
    fastify.delete(
        PROMPT_API.deletePrompt.pathTemplate,
        { schema: PROMPT_API.deletePrompt.schema },
        async (req, reply) => {
            const { promptId } = req.params as Static<typeof PromptParamsSchema>;
            const userId = currentUser().id;
