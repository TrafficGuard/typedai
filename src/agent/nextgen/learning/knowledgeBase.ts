...
export type { Learning, LearningSource, LearningType } from '../core/types';
...
`;
	...
	// ========================================================================
	// Code Style Retrieval Helpers (for AI Reviewer)
	// ========================================================================

	/**
	 * Gets code style learnings for specific file types
	 */
	async getCodeStyleForFileType(fileExtension: string): Promise<Learning[]> {
		await this.ensureInitialized();
		...
		const categoryMap: Record<string, string[]> = {
			...
			jsx: ['javascript', 'react', 'react/*'],
			py: ['python', 'python/*'],
			go: ['go', 'go/*'],
			rs: ['rust', 'rust/*'],
			java: ['java', 'java/*'],
			css: ['css', 'styling', 'styling/*'],
			scss: ['css', 'scss', 'styling', 'styling/*'],
		...
		const categories = categoryMap[ext] || [];
	...
	/**
	 * Gets pitfalls/anti-patterns for a technology
	 */
	async getPitfallsForTechnology(technology: string): Promise<Learning[]> {
		await this.ensureInitialized();

		return this.retrieve({
			categories: [technology.toLowerCase(), `${technology.toLowerCase()}/*`],
			types: ['pitfall'],
			minConfidence: 0.5,
			limit: 15,
		});
	...
	/**
	 * Gets design patterns for a category
	 */
	async getDesignPatterns(category: string): Promise<Learning[]> {
		await this.ensureInitialized();
	...
	/**
	 * Gets preferences/conventions for a project area
	 */
	async getPreferencesForArea(area: string): Promise<Learning[]> {
		await this.ensureInitialized();

		return this.retrieve({
			categories: [area.toLowerCase(), `${area.toLowerCase()}/*`],
			types: ['preference'],
			minConfidence: 0.6,
			limit: 10,
		});
	...
	/**
	 * Gets all code style learnings (patterns + preferences)
	 */
	async getAllCodeStyleLearnings(): Promise<Learning[]> {
		await this.ensureInitialized();
	...
	/**
	 * Searches for learnings by file paths
	 */
	async getLearningsForFiles(filePaths: string[]): Promise<Learning[]> {
		await this.ensureInitialized();
		...
		for (const filePath of filePaths) {
			...
			if (ext) {
				const styleLearnings = await this.getCodeStyleForFileType(ext);
				for (const learning of styleLearnings) {
					if (!seenIds.has(learning.id)) {
						seenIds.add(learning.id);
			...
			const pathCategories = this.inferCategoriesFromPath(filePath);
			if (pathCategories.length > 0) {
				const pathLearnings = await this.retrieve({
					categories: pathCategories,
					minConfidence: 0.6,
					limit: 5,
				});
				for (const learning of pathLearnings) {
					if (!seenIds.has(learning.id)) {
						seenIds.add(learning.id);
						allLearnings.push(learning);
					}
	...
	/**
	 * Infers categories from a file path
	 */
	private inferCategoriesFromPath(filePath: string): string[] {
		const categories: string[] = [];
		...
		for (const [pattern, category] of pathPatterns) {
			if (pattern.test(pathLower) && !categories.includes(category)) {
				categories.push(category);
			}
		}
	...
	/**
	 * Gets high-confidence learnings for review
	 */
	async getReviewLearnings(changedFiles: string[], taskDescription: string): Promise<Learning[]> {
		await this.ensureInitialized();

		// Get learnings from multiple sources and merge
		const fileLearnings = await this.getLearningsForFiles(changedFiles);
		const taskLearnings = await this.retrieveRelevant(taskDescription);

		// Merge and deduplicate
		const allLearnings = [...fileLearnings];
		const seenIds = new Set(fileLearnings.map((l) => l.id));

		for (const learning of taskLearnings) {
			if (!seenIds.has(learning.id)) {
				seenIds.add(learning.id);
				allLearnings.push(learning);
			}
		...
		return allLearnings.sort((a, b) => b.confidence - a.confidence).slice(0, 20);
	}
}
