...
export class AgentService {
	...
	private loadAgents(): void {
		...
		callApiRoute(this.http, AGENT_API.list)
			.pipe(
	...
	public loadAgentDetails(agentId: string): void {
		...
		callApiRoute(this.http, AGENT_API.details, { pathParams: { agentId } })
			.pipe(
				tap((agentDetails) => {
				...
				catchError((error) => {
					if (error?.status === 404) {
						...
						this._selectedAgentDetailsState.set({
							status: 'error',
							error: error instanceof Error ? error : new Error('Failed to load agent details'),
							code: error?.status,
						});
					}
					return EMPTY;
	...
	public loadLlmCalls(agentId: string): void {
		...
		callApiRoute(this.http, AGENT_API.getLlmCallSummaries, { pathParams: { agentId } })
			.pipe(
				tap((summaries) => {
					this._llmCallsState.set({ status: 'success', data: summaries as LlmCallSummary[] });
				}),
				catchError((error) => {
					this._llmCallsState.set({
						status: 'error',
						error: error instanceof Error ? error : new Error('Failed to load LLM call summaries'),
						code: error?.status,
					});
					return EMPTY;
				}),
	...
	public loadLlmCallDetail(agentId: string, llmCallId: string): void {
		...
		callApiRoute(this.http, AGENT_API.getLlmCallDetail, { pathParams: { agentId, llmCallId } })
			.pipe(
				tap((llmCallDetail) => {
				...
				catchError((error) => {
					if (error?.status === 404) {
						...
						this._selectedLlmCallDetailState.set({
							status: 'error',
							error: error instanceof Error ? error : new Error('Failed to load LLM call detail'),
							code: error?.status,
						});
					}
					return EMPTY;
	...
	public loadAgentIterations(agentId: string): void {
		...
		callApiRoute(this.http, AGENT_API.getIterationSummaries, { pathParams: { agentId } })
			.pipe(
				tap((summaries) => {
					this._agentIterationsState.set({ status: 'success', data: summaries as AutonomousIterationSummary[] });
				}),
				catchError((error) => {
					this._agentIterationsState.set({
						status: 'error',
						error: error instanceof Error ? error : new Error('Failed to load agent iteration summaries'),
						code: error?.status,
					});
					return EMPTY;
				}),
	...
	public loadAgentIterationDetail(agentId: string, iterationNumber: number): void {
		...
		callApiRoute(this.http, AGENT_API.getIterationDetail, { pathParams: { agentId, iterationNumber } })
			.pipe(
				tap((iterationDetail) => {
				...
				catchError((error) => {
					if (error?.status === 404) {
						...
						this._selectedAgentIterationDetailState.set({
							status: 'error',
							error: error instanceof Error ? error : new Error('Failed to load agent iteration detail'),
							code: error?.status,
						});
					}
					return EMPTY;
	...
	public loadAvailableFunctions(): void {
		...
		callApiRoute(this.http, AGENT_API.getAvailableFunctions)
			.pipe(
				map((response: string[]) => {
					// console.log('AgentService: fetched functions raw', response); // Retain existing comments if any, this one seems like a debug log
					const filteredAndSortedFunctions = response.filter((name) => name !== 'Agent').sort();
				...
				catchError((error) => {
					this._availableFunctionsState.set({
						status: 'error',
						error: error instanceof Error ? error : new Error('Failed to load available functions'),
						code: error?.status,
					});
					return EMPTY;
				}),
